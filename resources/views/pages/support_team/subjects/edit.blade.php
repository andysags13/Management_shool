...
@section('page_title', __('msg.edit_subject') . ' - ' . $s->name . ' (' . $s->my_class->name . ')')
@section('content')

    <div class="card">
        <div class="card-header header-elements-inline">
            <h6 class="card-title">{{ __('msg.edit_subject') }} - {{ $s->my_class->name }}</h6>
            {!! Qs::getPanelOptions() !!}
        </div>
                ...
                <div class="col-md-6">
                    <form class="ajax-update" method="post" action="{{ route('subjects.update', $s->id) }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-9">
                                <input name="name" value="{{ $s->name }}" required type="text" class="form-control" placeholder="{{ __('msg.name_of_subject') }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            <label for="my_class_id" class="col-lg-3 col-form-label font-weight-semibold">{{ __('msg.class') }} <span class="text-danger">*</span></label>
                            <div class="col-lg-9">
